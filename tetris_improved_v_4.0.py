...
invisible = (1,1,1,); silver = (211,211,211); light_grey = (196,196,196)
...
def display_statistics(surface):
    ...
    for i in range(7):
        ...
        surface.blit(percentage_label, (addition + 160 + top_left_x - 355 + play_width/2 - (percentage_label.get_width()/ 2),
                                70*i + top_left_y + play_height/2 - 190 - percentage_label.get_height()/2))
#"{:06.0f}"
        


def display_hud(surface, score, total_clears, tetris_rate, gravity):
    global fall_time
    global green_score
    global game_type
    global go
    ...
    surface.blit(score_label, (addition + top_left_x -290 + play_width/2 - (score_label.get_width()/ 2),
                         top_left_y + play_height/2 - 60 - score_label.get_height()/2))
    player_score = font.render("{:06.0f}".format(int(score)), 1, (255,255,0))
    surface.blit(player_score, (addition + top_left_x -290 + play_width/2 - (player_score.get_width()/ 2),
                         top_left_y + play_height/2 - 20 - player_score.get_height()/2))
    if leveled_up:
                draw_text_middle("Level Up!", 80, random.choice(shape_colors), surface)
    elif green_score > 0:
        draw_text_middle("+" + str(int(green_score)), 80, (0,255,0), surface)
    
    lines_label  = font.render("LINES", 1, (255, 255, 255))
    surface.blit(lines_label, (addition + top_left_x - 290 + play_width/2 - (lines_label.get_width()/ 2),
                               top_left_y + play_height/2 + 20 - lines_label.get_height()/2))    
    player_lines = font.render("{:03.0f}".format(total_clears), 1, (255,255,0))
    surface.blit(player_lines, (addition + top_left_x - 290 + play_width/2 - (player_lines.get_width()/ 2),
                               top_left_y + play_height/2 + 60 - player_lines.get_height()/2))           
    
    level_label  = font.render("LEVEL", 1, ((255, 255, 255)))
    surface.blit(level_label, (addition + top_left_x - 290 + play_width/2 - (level_label.get_width()/ 2),
                            top_left_y + play_height/2 + 100 - level_label.get_height()/2))      
    player_level = font.render(("{:02.0f}").format(level), 1, level_color)
    surface.blit(player_level, (addition + top_left_x - 290 + play_width/2 - (player_level.get_width()/ 2),
                                top_left_y + play_height/2 + 140 - player_level.get_height()/2))
    
    tr_label  = font.render("TETRIS RATE", 1, (255, 255, 255))
    surface.blit(tr_label, (addition + top_left_x - 290 + play_width/2 - (tr_label.get_width()/ 2),
                            top_left_y + play_height/2 + 180 - tr_label.get_height()/2))    
    player_tr = font.render(str(int(tetris_rate)) + "%", 1, (255,255,0))
    surface.blit(player_tr, (addition + top_left_x - 290 + play_width/2 - (player_tr.get_width()/ 2),
...
def draw_window(surface, shape):
    ...
    if leveled_up:
        label = font.render("TETRIS", 1, (0,0,255))
    elif lines_cleared == 4:
        label = font.render("TETRIS", 1, (255,215,0))
    else:
    ...
    display_hud(surface, score, total_clears, tetris_rate, gravity)
...
def game_over_part_2(grid, locked, win, next_shape, iteration):
    for x in range(10):
        locked[x, iteration] = (255,105,180)
...
def rotate(current_piece, grid, locked, direction): #direction is -1 or +1
    ...
    if game_type == "classic": return False #NES does not use wall-kick system
    for i in range(2,6):
...
def select_level():
    ...
    change = False 
    win.fill((0,0,0))
    draw_text_middle(("Level " + "{:02.0f}".format(level)), 60, (255,255,0), win) 
    pygame.display.update()
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                or event.key == pygame.K_a or event.key == pygame.K_a):
                    change = False
...
def select_game_type():
    def game_info():
        win.fill(black)
        font = pygame.font.SysFont('comicsans', 50)
        line1 = font.render('In classic tetris, pieces are randomly', 1, (orange))
        line2 = font.render('generated, and locking is instant.', 1, (orange))
        line3 = font.render('', 1, (255,255,255))
        line4 = font.render('In modern tetris, pieces are generated', 1, green)
        line5 = font.render('randomly in a seven-piece bag. You', 1, green)
        line6 = font.render('can hold pieces, along with other', 1, green)
        line7 = font.render('rotation and locking changes.', 1, green)
        line8 = font.render('Check readme for more information.', 1, green)
        lines = [line1, line2, line3, line4, line5, line6, line7, line8]
        for i in range(len(lines)):
            win.blit(lines[i], 
                         (top_left_x + play_width/2 - 325, 
                          top_left_y + play_height/2 - lines[i].get_height()/2 - 250 + 70*i))
        pygame.display.update()
        while True:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
    ...
    font = pygame.font.SysFont('comicsans', 60)
    prompt = font.render('Select Game Type', 1, (255,255,255))
    select1 = font.render("Classic", 1, (255,255,255))
    select2 = font.render("Modern", 1, (255,255,255))
        ...
        while True:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    quit()
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_y or event.key == pygame.K_c or event.key == pygame.K_m:
                        return True
    ...
    while run:
            ...
            if enabled:
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_p:
                        ...
                        pause()
                        fall_time = 0
                    if event.key == pygame.K_n and level > 0:
                        level -= 1
                    ...
                    if event.key == pygame.K_m and level < 12:
                        level += 1
        ...
        if change_piece:
            ...
            for i in range (lines_cleared):
                ...
                #     if level < 12:
                #         for j in range(12):
                #             if modern_level_list[j] == total_clears:
                #                 level += 1; gravity -= .035
                #                 level_gravity = gravity
            ...
            #     check_level = False
            #     if game_type == "modern":
            #         if starting_level <= 9 and total_clears >= starting_level * 10 + 10:
            #             level += 1
            #         elif starting_level <= 15 and total_clears >= 100:
            #             level += 1
            #         elif starting_level > 16 and total_clears >= starting_level * 10 - 10:
            #             level += 1  
            #     else:
            #         if total_clears == 10*starting_level + 10
            #         leveled_up = True
            #         print("Before, level/gravity is: ", level, gravity, starting_gravity)
            #         if level < 8: gravity = 1/2
            #         elif level == 8: gravity = 8/60
            #         elif level == 9: gravity = 6/60
            #         elif level > 9 and level < 13: gravity = 5/60
            #         elif level > 12 and level < 16: gravity = 4/60
            #         elif level > 15 and level < 19: gravity = 3/60
            #         elif level > 18 and level < 29: gravity = 2/60
            #         elif level > 28: gravity = 1/60
            #         starting_gravity = gravity
            #         level += 1
            #         print("After, Level/gravity is: ", level, gravity, starting_gravity)
            ...
            if game_type == "classic" or 0==0:
                ...
                if lines_cleared > 0:
                    if leveled_up:
                        to_color((turquoise), grid, locked_positions, 
                    ...
                    for i in range(3):
                        ...
                        if leveled_up:
                            to_color((255,255,255), grid, locked_positions, 
                                win, shape_list[0], pink_row_list)
                        elif lines_cleared == 4: ## gold
                            to_color((255,215,0), grid, locked_positions, 
                                win, shape_list[0], pink_row_list)
                        else:
                            to_color((255,105,180),grid, locked_positions,
                                            win, shape_list[0], pink_row_list)
                        grid = create_grid(locked_positions)
                        draw_window(win, -1)
                        draw_next_shape(shape_list[0], win)
            ...
            for i in range(lines_cleared):
                ...
                draw_next_shape(shape_list[0], win)
                draw_hold_shape(hold_piece, win)
                if game_type == "classic" or 0==0: pygame.display.update()
            lines_cleared = 0
            changed = True
            change_piece = False
            fall_time = 0
        ...
        if game_type == "classic": draw_next_shape(shape_list[0], win)
        else: draw_five_shapes(shape_list, win)
        draw_hold_shape(hold_piece, win)
        pygame.display.update()
        ...
        if check_lost(locked_positions):
            ...
            for i in range(20,-1,-1):
                game_over_part_2(grid, locked_positions, win, shape_list[0], i)
                grid = game_over_grid(locked_positions)
                draw_window(win, current_piece)
                draw_next_shape(shape_list[0], win)
                draw_hold_shape(hold_piece, win)
            ...
            pygame.display.update()
            pygame.time.delay(200)
            draw_text_middle("Final Score: " + str(int(score)), 40, (255,255,255), win)
            print(score)
            pygame.display.update()
...
def main_menu():
    try:
        ...
        while run:
            ...
            win.fill((0,0,0))
            font = pygame.font.SysFont('comicsans', 60)
            line1 = font.render('A -- Rotate Counterclockwise', 1, (255,255,255))
            line2 = font.render('S -- Rotate Clockwise', 1, (255,255,255))
            line3 = font.render('E -- Hold/Exchange Piece', 1, (255,255,255))
            line4 = font.render('P -- Pause', 1, (255,255,255))
            line5 = font.render('Press any key to continue.', 1, (255,255,255))
